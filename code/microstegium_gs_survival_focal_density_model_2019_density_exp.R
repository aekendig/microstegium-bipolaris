...
plotDens <- plotsD %>%
  mutate(mv_seedling_density = case_when(background == "Mv seedling" ~ background_density + 3, TRUE ~ 3),
         ev_seedling_density = case_when(background == "Ev seedling" ~ background_density + 3, TRUE ~ 3),
         ev_adult_density = case_when(background == "Ev adult" ~ background_density + 1, TRUE ~ 1))
...
# initial fit
mvGsSurvD2Mod1 <- brm(survival ~ fungicide * (mv_seedling_density + ev_seedling_density + ev_adult_density) + (1|site/plot),
                        data = mvGsSurvD2Dat,
                        family = bernoulli,
...
# simulation data
simDat <- tibble(mv_seedling_density = c(seq(3, 67, length.out = 100), rep(0, 200)),
                 ev_seedling_density = c(rep(0, 100), seq(3, 19, length.out = 100), rep(0, 100)),
                 ev_adult_density = c(rep(0, 200), seq(1, 9, length.out = 100)),
...
vizDat <- mvGsSurvD2Dat %>%
  ...
  full_join(plotsD2) %>%
  mutate(background_density = ifelse(background == "Ev adult", background_density + 1, background_density + 3))
...
save(mvGsSurvD2Mod2, file = "output/microstegium_gs_survival_focal_density_model_2019_density_exp.rda")
