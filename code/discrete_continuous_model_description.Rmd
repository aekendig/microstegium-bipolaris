...
lit_parms <- tibble(Parameter = c("beta_Al", "beta_Fl", "beta_Pl", 
                    ...
                    Description = c("litter transmission to annuals",
                     ...
                     Estimate = c(rep(0.01, 3), rep(10, 3), rep(0.001, 3), 0.5, 0.15, 0.05, 0, 0, 0.59),
...
```
We simulate the dynamics of each plant group alone and then all together over 160 days. Populations are initiated with 5000 plants and the equivalent of 100 infected annual plant for litter inoculum. We also simulated multiple starting values for plant density and litter inoculum.
...
xstart <- c(LogS_A = log(as.numeric(parms[parms$Parameter == "b_A", "Estimate"]) * 5000),
            ...
            l = as.numeric(parms[parms$Parameter == "b_A", "Estimate"]) * 100/10)
...
# input values
times <- seq(0, 160)
xstart <- c(LogS_A = log(1e-10), 
            LogS_F = log(as.numeric(parms[parms$Parameter == "b_F", "Estimate"]) * 5000),
            LogS_P = log(1e-10), I_A = 0, I_F = 0, I_P = 0,
            l = as.numeric(parms[parms$Parameter == "b_A", "Estimate"]) * 100/10)
...
xstart <- c(LogS_A = log(1e-10), 
            LogS_F = log(1e-10),
            LogS_P = log(as.numeric(parms[parms$Parameter == "b_P", "Estimate"]) * 5000), 
            I_A = 0, I_F = 0, I_P = 0,
            l = as.numeric(parms[parms$Parameter == "b_A", "Estimate"]) * 100/10)
...
#### start here: debug discrete model or simulation block (next one) ####
## Discrete time model
...
# input values
modA <- disc_mod_fun(AS0 = 100, AI0 = 0, F0 = 0, P0 = 0, L0 = 0, IA0 = 10, simtime = 10, gs_days = 160, disc_parms = parms, con_parms = parms)
