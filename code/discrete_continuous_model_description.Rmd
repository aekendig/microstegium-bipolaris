...
\begin{equation}
\begin{gathered}
dS_{i}/dt = r_{i}S_{i}(1 - \alpha_{iA}B_{A} - \alpha_{iF}B_{F} - \alpha_{iP}B_{P}) - \frac{\beta_{il}S_{i}l + \beta_{iA}S_{i}I_{A} + \beta_{iF}S_{i}I_{F} + \beta_{iP}S_{i}I_{P}}{k_{i} + B_{i}}
\\
...
lit_parms <- tibble(Parameter = c("beta_Al", "beta_Fl", "beta_Pl", 
                    ...
                    Description = c("litter transmission to annuals",
                     ...
                     Estimate = c(rep(0.11, 3), rep(10, 3), rep(0, 3), 1, 0.15, 0.05, 0, 0, 0.59),
...
cont_mod_fun <- function(t, x, params) {
  ...
  r_l <- as.numeric(params[params$Parameter == "r_l", "Estimate"])
  K_l <- as.numeric(params[params$Parameter == "K_l", "Estimate"])
  ...
  dLogSAdt <- r_A * (1 - alpha_AA * B_A - alpha_AF * B_F - alpha_AP * B_P) - (beta_Al * l + beta_AA * I_A + beta_AF * I_F + beta_AP * I_P)/(k_A + B_A)
  dLogSFdt <- r_F * (1 - alpha_FA * B_A - alpha_FF * B_F - alpha_FP * B_P) - (beta_Fl * l + beta_FA * I_A + beta_FF * I_F + beta_FP * I_P)/(k_F + B_F)
  dLogSPdt <- r_P * (1 - alpha_PA * B_A - alpha_PF * B_F - alpha_PP * B_P) - (beta_Pl * l + beta_PA * I_A + beta_PF * I_F + beta_PP * I_P)/(k_P + B_P)
  dIAdt <- (beta_Al * exp(LogS_A) * l + beta_AA * exp(LogS_A) * I_A + beta_AF * exp(LogS_A) * I_F + beta_AP * exp(LogS_A) * I_P)/(k_A + B_A) - v_A * I_A
  dIFdt <- (beta_Fl * exp(LogS_F) * l + beta_FA * exp(LogS_F) * I_A + beta_FF * exp(LogS_F) * I_F + beta_FP * exp(LogS_F) * I_P)/(k_F + B_F) - v_F * I_F
  dIPdt <- (beta_Pl * exp(LogS_P) * l + beta_PA * exp(LogS_P) * I_A + beta_PF * exp(LogS_P) * I_F + beta_PP * exp(LogS_P) * I_P)/(k_P + B_P) - v_P * I_P
  dldt <- r_l * l * (1 - l / K_l) - (beta_Al * exp(LogS_A) * l)/(k_A + B_A) - (beta_Fl * exp(LogS_F) * l)/(k_F + B_F) - (beta_Pl * exp(LogS_P) * l)/(k_P + B_P)
  ...
  # combine values
  dxdt <- c(dLogSAdt, dLogSFdt, dLogSPdt, dIAdt, dIFdt, dIPdt, dldt)
  
  # output
  list(dxdt)
}
```
We simulate the dynamics of each plant group alone and then all together over 160 days. Populations are initiated with 10 plants and the equivalent of 0.4 infected annual plant for litter inoculum. We also simulated multiple starting values for plant density and litter inoculum.
...
xstart <- c(LogS_A = log(as.numeric(parms[parms$Parameter == "b_A", "Estimate"]) * 5000),
            LogS_F = log(1e-10), LogS_P = log(1e-10), I_A = 0, I_F = 0, I_P = 0,
            l = as.numeric(parms[parms$Parameter == "b_A", "Estimate"]) * 75)
parms_in <- parms %>%
  full_join(tibble(Parameter = "K_l",
                   Estimate = as.numeric(parms[parms$Parameter == "b_A", "Estimate"]) * 750))

# apply function
out <- ode(y = xstart, times = times, func = cont_mod_fun, parms = parms_in) %>%
  as.data.frame()

# modify data table
outA <- out %>%
  as_tibble() %>%
  mutate(S_A = exp(LogS_A),
         S_F = exp(LogS_F),
         S_P = exp(LogS_P)) %>%
  select(-c(LogS_A, LogS_F, LogS_P)) %>%
...
# 
#   xstart <- c(LogS_A = log(as.numeric(parms[parms$Parameter == "b_A", "Estimate"]) * starts[i]),
#             LogS_F = log(1e-10), LogS_P = log(1e-10), I_A = 0, I_F = 0, I_P = 0,
#             l = as.numeric(parms[parms$Parameter == "b_A", "Estimate"]) * 0.04)
# 
#   # apply function
...
#   parms_in <- parms %>%
#   full_join(tibble(Parameter = "K_l",
#                    Estimate = as.numeric(parms[parms$Parameter == "b_A", "Estimate"]) * starts[i]))
# 
#   xstart <- c(LogS_A = log(as.numeric(parms[parms$Parameter == "b_A", "Estimate"]) * as.numeric(parms[parms$Parameter == "g_S", "Estimate"]) * as.numeric(parms[parms$Parameter == "e_A", "Estimate"]) * 5000),
#             LogS_F = log(1e-10), LogS_P = log(1e-10), I_A = 0, I_F = 0, I_P = 0,
#             l = as.numeric(parms[parms$Parameter == "b_A", "Estimate"]) * starts[i]/10)
# 
#   # apply function
#   out <- ode(func = cont_mod_fun, y = xstart, times = times, parms = parms_in) %>%
#     as.data.frame()
...
```
START HERE: Why doesn't infected biomass reach a carrying capacity of 1/alpha? Why isn't the inoculum depleted with all this transmission?
...
xstart <- c(S_A = as.numeric(parms[parms$Parameter == "b_A", "Estimate"]) * 10, 
            ...
            S_P = as.numeric(parms[parms$Parameter == "b_P", "Estimate"]) * 10, 
            I_A = 0, I_F = 0, I_P = 0,
            l = as.numeric(parms[parms$Parameter == "b_A", "Estimate"]) * 0)
...
disc_mod_fun = function(AS0, AI0, F0, P0, L0, IA0, simtime, gs_days, disc_parms, con_parms){
  ...
  for(t in 1:(simtime - 1)){	
    ...
    out <- ode(func = cont_mod_fun,
               y = c(LogS_A = log(g_S * E_A * b_A * A_S[t] + g_I * E_A * b_A * A_I[t]),
                     LogS_F = log(g_P * E_P * b_F * F1[t]),
                     LogS_P = log(b_P * P[t]),
                     I_A = 0, I_F = 0, I_P = 0,
                     l = I_A0[t]/10),
               times = grow_days,
    ...
    # population size
    A_S[t+1] = s_A * (1 - g_S) * A_S[t] + (1-p) * c_A * B_A[t]
    A_I[t+1] = p * c_A * B_A[t]
    L[t+1] = (1 - d) * L[t] + B_A[t]   
  ...
  # seedling establishment
    ...
    con_parms <- con_parms %>%
      full_join(tibble(Parameter = "K_l",
                       Estimate = I_A0[simtime]))
    
    # biomass at the end of the growing season
    out3 <- ode(func = cont_mod_fun,
               y = c(LogS_A = log(g_S * E_A * b_A * A_S[simtime] + g_I * E_A * b_A * A_I[simtime]),
                     LogS_F = log(g_P * E_P * b_F * F1[simtime]),
                     LogS_P = log(b_P * P[simtime]),
                     I_A = 0, I_F = 0, I_P = 0,
                     l = I_A0[simtime]/10),
               times = grow_days,
               parms = con_parms) %>%
  ...
  dfN <- tibble(time = rep(1:simtime, 14),
                species = rep(c("Susceptible annual",
                              ...
                              each = simtime),
                N = c(A_S, A_I, F1, P, L, B_A, B_F, B_P, S_A, S_F, S_P, I_A, I_F, I_P))
...
# input values
modA <- disc_mod_fun(AS0 = 20, AI0 = 0, F0 = 0, P0 = 0, L0 = 0, IA0 = 1, simtime = 10, gs_days = 160, con_parms = parms)
...
# input values
modF <- disc_mod_fun(AS0 = 0, AI0 = 0, F0 = 20, P0 = 0, L0 = 0, IA0 = 0, simtime = 100, gs_days = 160, con_parms = parms)
...
# input values
modP <- disc_mod_fun(AS0 = 0, AI0 = 0, F0 = 0, P0 = 20, L0 = 0, IA0 = 0, simtime = 100, gs_days = 160, con_parms = parms)
...
# input values
modT <- disc_mod_fun(AS0 = 10, AI0 = 0, F0 = 10, P0 = 10, L0 = 0, IA0 = 1, simtime = 100, gs_days = 160, con_parms = parms)
...
To simulate historical future dynamics, we start with a community consisting of only the perennial species. The annual species invades after 100 years, but there are is no disease (i.e., $\beta_{Al} = \beta_{Fl} = \beta_{Pl} = 0$, which prevents initial infection, and $p = 0$). After 100 more years, the annual begins to spread disease within the community (i.e., $\beta_{Al} = \beta_{Fl} = \beta_{Pl} = 0.1$). We begin with 10 first-year perennials and 10 adult perennials and later introduce 10 annuals.
...
# perennial only
modL1 <- disc_mod_fun(AS0 = 10, AI0 = 0, F0 = 0, P0 = 0, L0 = 0, IA0 = 0, simtime = 50, gs_days = 160, 
...
modL2 <- disc_mod_fun(AS0 = modL2_in$AS0,
                      AI0 = modL2_in$AI0,
                      F0 = modL2_in$F0,
                      P0 = modL2_in$P0,
                      L0 = modL2_in$L0,
                      IA0 = modL2_in$IA0,
                      simtime = 10, gs_days = 160, disc_parms = parms_df, con_parms = parms_df)

# input values for next simulation
modL3_in <- modL2 %>%
  filter(time == max(time) &
           species %in% c("Susceptible annual", "Infected annual", "Perennial first-year", "Perennial adult", "Litter")) %>%
  select(-time) %>%
  pivot_wider(names_from = species,
              values_from = N) %>%
  rename(AS0 = "Susceptible annual",
...
parms_df2 <- parms %>%
  mutate(Estimate = case_when(Parameter == "p1" ~ 0,
                              Parameter == "p0" ~ -100,
                              TRUE ~ Estimate))
...
modL3 <- disc_mod_fun(AS0 = modL3_in$AS0,
                      ...
                      L0 = modL3_in$L0,
                      IA0 = 1,
                      simtime = 10, gs_days = 160, disc_parms = parms_df2, con_parms = parms_df2)
