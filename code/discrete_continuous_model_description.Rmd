...
\begin{equation}
\begin{gathered}
dS_{i}/dt = r_{i}S_{i}(1 - \alpha_{iA}B_{A} - \alpha_{iF}B_{F} - \alpha_{iP}B_{P}) - \frac{\beta_{il}S_{i}l + \beta_{iA}S_{i}I_{A} + \beta_{iF}S_{i}I_{F} + \beta_{iP}S_{i}I_{P}}{k_{i} + S_{i}}
\\
dI_{i}/dt = \frac{\beta_{il}S_{i}l + \beta_{iA}S_{i}I_{A} + \beta_{iF}S_{i}I_{F} + \beta_{iP}S_{i}I_{P}}{k_{i} + S_{i}} - v_{i}I_{i}
\\
dl/dt = -hl
\\
...
lit_parms <- tibble(Parameter = c("beta_Al", "beta_Fl", "beta_Pl", 
                    ...
                    Description = c("litter transmission to annuals",
                     ...
                     Estimate = c(rep(0.11, 3), rep(10, 3), rep(0, 3), 0.1, 0.15, 0.05, 0, 0, 0.59),
...
cont_mod_fun <- function(t, x, params) {
  ...
  # model with type-II transmission
  dSAdt <- r_A * S_A * (1 - alpha_AA * B_A - alpha_AF * B_F - alpha_AP * B_P) -
    (beta_Al * S_A * l + beta_AA * S_A * I_A + beta_AF * S_A * I_F + beta_AP * S_A * I_P)/(k_A + S_A)
  dSFdt <- r_F * S_F * (1 - alpha_FA * B_A - alpha_FF * B_F - alpha_FP * B_P) -
    (beta_Fl * S_F * l + beta_FA * S_F * I_A + beta_FF * S_F * I_F + beta_FP * S_F * I_P)/(k_F + S_F)
  dSPdt <- r_P * S_P * (1 - alpha_PA * B_A - alpha_PF * B_F - alpha_PP * B_P) -
    (beta_Pl * S_P * l + beta_PA * S_P * I_A + beta_PF * S_P * I_F + beta_PP * S_P * I_P)/(k_P + S_P)
  dIAdt <- (beta_Al * S_A * l + beta_AA * S_A * I_A + beta_AF * S_A * I_F + beta_AP * S_A * I_P)/(k_A + S_A) - v_A * I_A
  dIFdt <- (beta_Fl * S_F * l + beta_FA * S_F * I_A + beta_FF * S_F * I_F + beta_FP * S_F * I_P)/(k_F + S_F) - v_F * I_F
  dIPdt <- (beta_Pl * S_P * l + beta_PA * S_P * I_A + beta_PF * S_P * I_F + beta_PP * S_P * I_P)/(k_P + S_P) - v_P * I_P
  dldt <- -h * l
...
```
We simulate the dynamics of each plant group alone and then all together over 160 days. Populations are initiated with 10 plants and the equivalent of 10 annual plants for litter. We also simulated multiple starting values for plant density
...
#   
#   xstart <- c(S_A = as.numeric(parms[parms$Parameter == "b_A", "Estimate"]) * starts[i],
#             S_F = 0, S_P = 0, I_A = 0, I_F = 0, I_P = 0,
#             l = as.numeric(parms[parms$Parameter == "b_A", "Estimate"]))
# 
# # apply function
# ode(
#   func = cont_mod_fun,
...
disc_mod_fun = function(AS0, AI0, F0, P0, L0, IA0, simtime, gs_days, con_parms){
  ...
  for(t in 1:(simtime - 1)){	
    ...
    ode(
      func = cont_mod_fun,
      y = c(S_A = g_S * E_A * b_A * A_S[t] + g_I * E_A * b_A * A_I[t], 
    ...
    I_A[t] <- ifelse(I_A[t] < 0, 0, I_A[t])
    I_F[t] <- ifelse(I_F[t] < 0, 0, I_F[t])
    I_P[t] <- ifelse(I_P[t] < 0, 0, I_P[t])
    ...
    if(B_A[t] > 0) {
      p <- exp(p0 + p1 * I_A[t]/B_A[t])/(1 + exp(p0 + p1 * I_A[t]/B_A[t]))
      } else {
        p <- 0
        }
...
# input values
modA <- disc_mod_fun(AS0 = 20, AI0 = 0, F0 = 0, P0 = 0, L0 = 0, IA0 = 1, simtime = 100, gs_days = 160, con_parms = parms)
...
modA %>%
  ...
  labs(x='Time (years)', y='Biomass (g)')
```
#### Start here: want infected biomass to asymptote lower/around where susceptible biomass does...
...
# perennial only
modL1 <- disc_mod_fun(AS0 = 0, AI0 = 0, F0 = 10, P0 = 10, L0 = 0, IA0 = 0, simtime = 100, gs_days = 160, con_parms = parms_df)
...
modL2 <- disc_mod_fun(AS0 = 10, 
                      ...
                      L0 = modL2_in$L0, 
                      IA0 = modL2_in$IA0,
                      simtime = 100, gs_days = 160, con_parms = parms_df)
...
modL3 <- disc_mod_fun(AS0 = modL3_in$AS0, 
                      ...
                      L0 = modL3_in$L0, 
                      IA0 = 1,
                      simtime = 100, gs_days = 160, con_parms = parms)
