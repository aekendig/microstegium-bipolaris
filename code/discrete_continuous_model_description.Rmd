...
```
#### start here
The largest biomass decrease above is 3%. With higher virulence, the disease cannot be sustained. Should parameters be adjusted to sustain the disease or should infected biomass be not allowed to contribute to total biomass growth? Looking for a ~30% decrease.
...
# perennial only
modL1 <- disc_mod_fun(AS0 = 1e-100, AI0 = 0, F0 = 10, P0 = 10, L0 = 0, IA0 = 0, simtime = 100, gs_days = 160, 
...
### Total annual density
```{r long-term simulation total}
modL_tot <- modL %>%
  filter(species %in% c("Susceptible annual", "Infected annual", "Perennial first-year", "Perennial adult")) %>%
  pivot_wider(names_from = species,
              values_from = N) %>%
  rename(AS = "Susceptible annual",
         AI = "Infected annual",
         F1 = "Perennial first-year",
         P = "Perennial adult") %>%
  mutate(A = AS + AI) %>%
  pivot_longer(cols = c(A, AS, AI, F1, P),
               names_to = "species",
               values_to = "N") %>%
  filter(!(species %in% c("AS", "AI")))

ggplot(modL_tot, aes(x = time, y = N, color = species)) +
  geom_line(size = 2) +
  theme_classic() +
  labs(x='Time (years)', y='Number of plants')

# total population drops very slightly
```
