...
# setting lower bounds for k and alpha parameters (don't know why this is necessary, should
# be implicit in prior), as well constraining the k parameter (using a more informed prior)
# eliminates divergent transitions
...
ea_seeds_ea_bg_mod <- brm(data = ea_ea_dat, family = gaussian,
                          bf(seeds ~ s0 + background_density / (k + alpha * background_density^2),
                             ...
                             alpha ~ fungicide,
                             nl = T),
                          prior <- c(prior(normal(100, 200), nlpar = "s0"),
                                     prior(exponential(1), nlpar = "k", lb = 0),
                                     prior(exponential(5), nlpar = "alpha", lb = 0)),
                          iter = 2000, warmup = 1000, chains = 2, cores = 1,
                          control = list(adapt_delta = 0.9))
# Also note that site effects eliminate funcigide influence on k and alpha, but not s0
print(ea_seeds_ea_bg_mod)

## Account for Uncertainty surrounding seeds per gram spikelet weight ###
# calculate sd on regression residuals (here using all data, but adjust as you feel appropriate):
seed_wt = data.frame(spikelet_wt = c(spike18$spikelet_weight.g, spike19$spikelet_weight.g),
                     seed_count = c(spike18$seeds, spike19$seeds), 
                     weighed = c(!is.na(spike18$seed_weight.mg), !is.na(spike19$seed_weight)))
seed_wt = seed_wt %>% 
  filter(weighed == T, seed_count > 0)
sd(resid(lm(seed_count ~ spikelet_wt, seed_wt)))

latent_seeds <- brm(data = ea_ea_dat, family = gaussian,
                    bf(seeds | mi(17) ~ s0 + background_density / (k + alpha * background_density^2),
                       s0 ~ fungicide + (1|site),
                       k ~ fungicide,
                       alpha ~ fungicide,
                       nl = T),
                    ...
                    iter = 2000, warmup = 1000, chains = 2, cores = 1,
                    control = list(adapt_delta = 0.9))
# Only 1 divergent transition even with fungicide and site effects
print(latent_seeds)

# to verify that noise has been added to the response, run:
brms::stancode(latent_seeds)
# where key term is:
