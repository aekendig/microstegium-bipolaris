...
mean_fun <- function(mod, trans, plant_type, response){
  
  out <- posterior_samples(mod) %>%
    as_tibble() %>%
    mutate(ctrl = case_when(trans == T ~ exp(b_Intercept) / (1 + exp(b_Intercept)),
                            trans == F ~ b_Intercept),
           fung = case_when(trans == T ~ exp(b_Intercept + b_fungicide) / (1 + exp(b_Intercept + b_fungicide)),
                            trans == F ~ b_Intercept + b_fungicide), 
           fung_eff = (fung - ctrl) / ctrl) %>%
    select(fung_eff) %>%
    mean_hdci() %>%
...
# Used hdci because Ev adult had two hdi's, the one that is not well represented with hdci didn't make much sense anyway (lower and upper don't include effect). All other models are the same with hdci or hdi.
...
fung_fig <- ggplot(mod_eff2, aes(x = plant_type, y = fung_eff)) +
  ...
  ylab("Fungicide effect: (fungicide - control) / fungicide") +
