...
# sample parameters
B_P_dens <- evABioSamps[sample(nrow(evABioSamps), size = n_samps, replace = T), ] %>%
...
B_P_fun <- function(disease, g.A, E.A, A_dens, g.S, E.S, S_dens, P_dens, iter) {
  
  # calculate survival
  B_P_expr <- ifelse(disease == 1, 
                     B_P_dens$int_wat[iter] - log(1 + B_P_dens$mv_dens_wat[iter] * g.A * E.A * A_dens + B_P_dens$evS_dens_wat[iter] * g.S * E.S * S_dens + B_P_dens$evA_dens_wat[iter] * P_dens),
                     B_P_dens$int_fun[iter] - log(1 + B_P_dens$mv_dens_fun[iter] * g.A * E.A * A_dens + B_P_dens$evS_dens_fun[iter] * g.S * E.S * S_dens + B_P_dens$evA_dens_fun[iter] * P_dens))
