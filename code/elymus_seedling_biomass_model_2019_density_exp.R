...
# initial fit
evSBioMod1 <- brm(bf(bio.g ~ v / (1 + alphaA * mv_seedling_density + alphaS * ev_seedling_density + alphaP * ev_adult_density),
                     v ~ fungicide + treatment + (1|site),
                     alphaA ~ 0 + treatment,
                     alphaS ~ 0 + treatment,
                     alphaP ~ 0 + treatment,
                     nl = T),
                  data = evSBioD2Dat2, family = gaussian,
                  prior <- c(prior(normal(4, 10), nlpar = "v", class = "b", coef = "Intercept"),
                             prior(normal(0.04, 0.001), nlpar = "v", class = "b", coef = "treatmentfungicide"),
                             prior(normal(0, 10), nlpar = "v", class = "b"),
                             prior(exponential(0.5), nlpar = "alphaA", lb = 0),
                             prior(exponential(0.5), nlpar = "alphaS", lb = 0),
                             prior(exponential(0.5), nlpar = "alphaP", lb = 0),
                             prior(cauchy(0, 1), nlpar = "v", class = "sd"),
                             prior(cauchy(0, 1), class = "sigma")),
                  iter = 6000, warmup = 1000, chains = 1)
...
# simulate fit
fitDat <- simDat %>%
  mutate(bio.g = fitted(evSBioMod2, newdata = ., re_formula = NA, type = "response")[, "Estimate"],
         bio.g_lower = fitted(evSBioMod2, newdata = ., re_formula = NA, type = "response")[, "Q2.5"],
         bio.g_upper = fitted(evSBioMod2, newdata = ., re_formula = NA, type = "response")[, "Q97.5"])
