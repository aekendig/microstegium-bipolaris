...
# simulate fit
fitDat <- simDat %>%
  mutate(log_bio.g = fitted(evSBioMod2, newdata = ., re_formula = NA)[, "Estimate"],
         log_bio.g_lower = fitted(evSBioMod2, newdata = ., re_formula = NA)[, "Q2.5"],
         log_bio.g_upper = fitted(evSBioMod2, newdata = ., re_formula = NA)[, "Q97.5"],
         treatment = ifelse(treatment == "control", "water", treatment))

# change levels on raw data
evSBioD2Dat3 <- evSBioD2Dat2 %>%
  mutate(treatment = ifelse(treatment == "control", "water", treatment))

# fit figure
evSBioPlot <- ggplot(fitDat, aes(background_density, log_bio.g, color = treatment, fill = treatment)) +
  geom_ribbon(aes(ymin = log_bio.g_lower, ymax = log_bio.g_upper), alpha = 0.5, color = NA) +
  geom_line(size = 1.5) +
  stat_summary(data = evSBioD2Dat3, geom = "errorbar", width = 0, fun.data = "mean_cl_boot") +
  stat_summary(data = evSBioD2Dat3, geom = "point", size = 2, fun = "mean") +
  facet_wrap(~ background, scales = "free_x") +
  theme_bw()
...
save(evSBioPlot, file = "output/elymus_seedling_biomass_figure_2019_density_exp.rda")
