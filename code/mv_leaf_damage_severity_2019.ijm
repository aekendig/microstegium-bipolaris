// to run parts of code: cmd+shift+R
...
home="/Users/AmyKendig/Dropbox (UFL)/big-oaks-field-experiment-2018-2019/leaf-scans/leaf-scans-jul-2019-density-exp";
//enter input folder path
input=home + "/scans/temp"; 
...
outputResTxt=outputRes + "/mv_leaf_scan_text_output_may_2019_density_exp2.tsv";
...
function processFile(input, outputIm, outputRes, file) {
	...
	filter[0]="pass";
	min[1]=131;
	max[1]=169; // a* is 131 to 169
	filter[1]="pass";
	min[2]=138;
	max[2]=169; // b* is 138 to 169
	filter[2]="pass";
	for (i=0;i<3;i++){
	  selectWindow(""+i);
	  setThreshold(min[i], max[i]);
