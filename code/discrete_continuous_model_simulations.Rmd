...
growth_mod_parms <- posterior_samples(growthD2Mod2)  %>%
  ...
  transmute(r_A = Intercept,
            r_F = Intercept + focs,
            r_P = Intercept + foca,
            alpha_AA = - 1 * (plot_biomass + plot_biomass_fungicide),
            # alpha_AA = 1/max(stricker$biomass_g_m2),
            alpha_FA = - 1 * (plot_biomass + plot_biomass_fungicide + plot_biomass_focs + plot_biomass_focs_fungicide),
            alpha_PA = - 1 * (plot_biomass + plot_biomass_fungicide + plot_biomass_foca + plot_biomass_foca_fungicide),
            alpha_AF = - 1 * (plot_biomass + plot_biomass_bgs + plot_biomass_fungicide + plot_biomass_bgs_fungicide),
            alpha_FF = - 1 * (plot_biomass + plot_biomass_focs + plot_biomass_bgs + plot_biomass_focs_bgs + plot_biomass_fungicide + plot_biomass_focs_fungicide + plot_biomass_bgs_fungicide + plot_biomass_focs_bgs_fungicide),
            alpha_PF = - 1 * (plot_biomass + plot_biomass_foca + plot_biomass_bgs + plot_biomass_foca_bgs + plot_biomass_fungicide + plot_biomass_foca_fungicide + plot_biomass_bgs_fungicide + plot_biomass_foca_bgs_fungicide),
            alpha_AP = - 1 * (plot_biomass + plot_biomass_bga + plot_biomass_fungicide + plot_biomass_bga_fungicide),
            alpha_FP = - 1 * (plot_biomass + plot_biomass_bga + plot_biomass_focs + plot_biomass_focs_bga + plot_biomass_fungicide + plot_biomass_bga_fungicide + plot_biomass_focs_fungicide + plot_biomass_focs_bga_fungicide),
            alpha_PP = - 1 * (plot_biomass + plot_biomass_foca + plot_biomass_bga + plot_biomass_foca_bga + plot_biomass_fungicide + plot_biomass_foca_fungicide + plot_biomass_bga_fungicide + plot_biomass_foca_bga_fungicide),
            alpha_AA_dis = - 1 * plot_biomass,
            alpha_FA_dis = - 1 * (plot_biomass + plot_biomass_focs),
            alpha_PA_dis = - 1 * (plot_biomass + plot_biomass_foca),
            alpha_AF_dis = - 1 * (plot_biomass + plot_biomass_bgs),
            alpha_FF_dis = - 1 * (plot_biomass + plot_biomass_focs + plot_biomass_bgs + plot_biomass_focs_bgs),
            alpha_PF_dis = - 1 * (plot_biomass + plot_biomass_foca + plot_biomass_bgs + plot_biomass_foca_bgs),
            alpha_AP_dis = - 1 * (plot_biomass + plot_biomass_bga),
            alpha_FP_dis = - 1 * (plot_biomass + plot_biomass_bga + plot_biomass_focs + plot_biomass_focs_bga),
            alpha_PP_dis = - 1 * (plot_biomass + plot_biomass_foca + plot_biomass_bga + plot_biomass_foca_bga)) %>%
...
evA_surv_parms <- posterior_samples(adultSurvD1Mod)  %>%
  transmute(l = exp(b_Intercept)/(1 + exp(b_Intercept))) %>%
  pivot_longer(cols = everything(),
               names_to = "parameter",
               values_to = "estimate") %>%
  group_by(parameter) %>%
...
lit_parms <- tibble(Parameter = c("beta_Al", "beta_Fl", "beta_Pl", 
                    ...
                    Description = c("litter transmission to annuals",
                     ...
                     Estimate = c(rep(0.001, 3), rep(10, 3), rep(1e-20, 3), 5500, 0.1, 0.15, 0.05, 0, 0, 0.59),
                     Source = c(rep("NA", 9), "Benitez et al. 2020", "NA", "Redwood et al. 2018", "Garrison and Stier 2010", "NA", "NA", "DeMeester and Richter 2010")) %>%
  mutate(Lower = NA_real_,
         Upper = NA_real_,
         Units = c(rep ("day^-1^ g^-1^", 3), rep ("g", 3), rep("day^-1^", 3), "g^-1^", "day^-1^", rep("NA", 2), rep("g^-1^", 2), "NA"))
...
cont_mod_fun <- function(t, x, params) {
  ...
  m <- as.numeric(params[params$Parameter == "m", "Estimate"])
  ...
  dIAdt <- (beta_AC * S_A * C + beta_AA * S_A * I_A + beta_AF * S_A * I_F + beta_AP * S_A * I_P)/(k_A + B_A) - v_A * I_A
  dIFdt <- (beta_FC * S_F * C + beta_FA * S_F * I_A + beta_FF * S_F * I_F + beta_FP * S_F * I_P)/(k_F + B_F) - v_F * I_F
  dIPdt <- (beta_PC * S_P * C + beta_PA * S_P * I_A + beta_PF * S_P * I_F + beta_PP * S_P * I_P)/(k_P + B_P) - v_P * I_P
  dDdt <- v_A * I_A
  dCdt <- h * D - m * C
...
disc_mod_fun = function(AS0, AI0, F0, P0, L0, C0, IA0, SP0, SF0, simtime, gs_days, disc_parms, con_parms){
  ...
  l <- as.numeric(parms[parms$Parameter == "l", "Estimate"])
  ...
  for(t in 1:(simtime - 1)){	
    ...
    S_P0[t+1] <- S_P[t] # perennial biomass
    S_F0[t+1] <- S_F[t] # perennial biomass
  ...
  # seedling establishment
    ...
    out3 <- ode(func = cont_mod_fun,
               y = c(LogB_A = log(g_S * E_A * b_A * A_S[simtime] + g_I * E_A * b_A * A_I[simtime]),
                     ...
                     LogB_P = log(l * (S_P0[simtime] + S_F0[simtime])),
                     I_A = 0, I_F = 0, I_P = 0, D = 0,
...
# input values
modF <- disc_mod_fun(AS0 = 1e-100, AI0 = 0, F0 = 10, P0 = 1e-10, L0 = 0, IA0 = 1, simtime = 100, gs_days = 160, disc_parms = parms, con_parms = parms)
