...
# adjustment for competition coefficients (including litter sensitivity)
alpha_mult <- 1
alpha_add <- 1e-4
surv_adjust <- 1

# growth rates and competition coefficients
growth_mod_parms <- as_draws_df(growthD2Mod2)  %>%
  ...
  mutate(estimate = case_when(substr(parameter, 1, 1) == "r" ~ estimate,
                              TRUE ~ estimate * -1)) %>% # competition coefficients
  group_by(parameter) %>%
  mean_hdi(estimate) %>%
  transmute(Parameter = parameter,
            ...
            Estimate = case_when(parameter %in% c("r_A", "r_F", "r_P") ~ estimate/160, # r may not be sig, but we need an estimate
                                 .lower < 0 & .upper > 0 ~ 0,
                                 estimate < 0 ~ 0, # remove facilitation
                                 TRUE ~ estimate),
...
growth_mod_parms2 <- growth_mod_parms %>%
  mutate(Estimate = case_when(substr(Parameter, 1, 5) == "alpha" ~ (Estimate + alpha_add) * alpha_mult, # alpha adjustments
                                 TRUE ~ Estimate))
...
lit_parms <- tibble(Parameter = c("beta_AC", "beta_FC", "beta_PC", 
                    ...
                    Description = c("litter transmission to annuals",
                     ...
                     Estimate = c(rep(0.001, 3), rep(10, 3), rep(1e-4, 3), 5500, 0.5, 0.15, 0.05, 0.59, rep(0.003, 3)),
...
# continuous models
source("../code/continuous_both_species_model.R")
source("../code/continuous_annual_model.R")
source("../code/continuous_perennial_model.R")
```
...
# apply function
out <- ode(y = xstart, times = times, func = cont_annual_mod, parms = parms) %>%
  as.data.frame()
...
for(i in 1:length(starts)){
  ...
  # apply function
  out <- ode(func = cont_annual_mod, y = xstart, times = times, parms = parms) %>%
    as.data.frame()
...
for(i in 1:length(vir)){
  ...
  # apply function
  out <- ode(func = cont_annual_mod, y = xstart, times = times, parms = parms_in) %>%
    as.data.frame()
...
xstart <- c(LogB_F = log(as.numeric(parms[parms$Parameter == "b_F", "Estimate"]) * 10),
            LogB_P = log(1e-10), I_F = 0, I_P = 0, D = 0,
            C = as.numeric(parms[parms$Parameter == "h", "Estimate"]))

# apply function
ode(
  func = cont_perennial_mod,
  y = xstart,
  times = times,
  parms = parms
) %>%
...
# input values
times <- seq(0, gs_time)
xstart <- c(# LogB_A = log(0.767 * 0.949/(1 + 0.0012 * 3604) * 0.031 * 7.73e-146),
            LogB_A = log(1e-10),
            LogB_F = log(0.24 * 0.553/(1 + 5e-4 * 3604) * 0.004 * 45060),
            LogB_P = log(0.88 * (7.7 + 2268)), 
            # LogB_P = log(0.88 * 100), 
            I_A = 0, 
            I_F = 0, I_P = 0, D = 0,
            C = 131 + as.numeric(parms[parms$Parameter == "h", "Estimate"]) * (7.66 + 111))
...
source("../code/discrete_both_species_model.R")
source("../code/discrete_annual_model.R")
source("../code/discrete_perennial_model.R")
```
...
# input values
modA <- disc_annual_mod(AS0 = 10, AI0 = 0, L0 = 0, C0 = as.numeric(parms[parms$Parameter == "h", "Estimate"]), IA0 = 0, simtime = 50, gs_days = gs_time, disc_parms = parms, con_parms = parms)
...
# input values
modF <- disc_perennial_mod(F0 = 10, P0 = 0, L0 = 0, C0 = as.numeric(parms[parms$Parameter == "h", "Estimate"]), IF0 = 0, IP0 = 0, BP0 = 0, BF0 = 0, simtime = 50, gs_days = gs_time, disc_parms = parms, con_parms = parms)
...
# input values
modP <- disc_perennial_mod(F0 = 0, P0 = 10, L0 = 0, C0 = as.numeric(parms[parms$Parameter == "h", "Estimate"]), IF0 = 0, IP0 = 0, BP0 = as.numeric(parms[parms$Parameter == "b_P", "Estimate"]) * 10, BF0 = 0, simtime = 100, gs_days = gs_time, disc_parms = parms, con_parms = parms)
...
# perennial only
modL1 <- disc_perennial_mod(F0 = 100, P0 = 0, L0 = 0, C0 = 0, IF0 = 0, IP0 = 0, BP0 = 0, BF0 = 0, simtime = 100, gs_days = gs_time, disc_parms = params_df, con_parms = params_df)

# input values for next simulation
modL2_in <- modL1 %>%
  filter(time == max(time) &
           species %in% c("Perennial first-year", "Perennial adult", "Perennial adult biomass", "Perennial first-year biomass", "Litter")) %>%
  select(-time) %>%
...
# perennial only
modL2_per <- disc_perennial_mod(F0 = modL2_in$F0,
                      P0 = modL2_in$P0,
                      L0 = modL2_in$L0,
                      C0 = 0,
                      IF0 = 0, 
...
modL2_inv <- disc_both_mod(AS0 = 100,
                      AI0 = 0,
                      F0 = modL2_in$F0,
                      P0 = modL2_in$P0,
                      L0 = modL2_in$L0,
...
modL3_in_inv <- modL2_inv %>%
  filter(time == max(time) &
           species %in% c("Susceptible annual", "Perennial first-year", "Perennial adult", "Litter", "Annual biomass", "Perennial adult biomass", "Perennial first-year biomass")) %>%
  select(-time) %>%
  pivot_wider(names_from = species,
              values_from = N) %>%
...
# perennial only
modL3_per <- disc_perennial_mod(F0 = modL3_in_per$F0,
                      P0 = modL3_in_per$P0,
                      L0 = modL3_in_per$L0,
                      C = 0,
                      IF0 = 0, 
...
# invasion only
modL3_inv <- disc_both_mod(AS0 = modL3_in_inv$AS0,
                      AI0 = 0,
                      F0 = modL3_in_inv$F0,
                      P0 = modL3_in_inv$P0,
                      L0 = modL3_in_inv$L0,
...
# disease appears
modL3_dis <- disc_both_mod(AS0 = modL3_in_inv$AS0,
                      AI0 = 0,
                      F0 = modL3_in_inv$F0,
                      P0 = modL3_in_inv$P0,
                      L0 = modL3_in_inv$L0,
                      C = as.numeric(params[params$Parameter == "h", "Estimate"]),
                      IA0 = 0,
                      IF0 = 0, 
                      IP0 = 0,
                      BP0 = modL3_in_inv$BP0,
                      BF0 = modL3_in_inv$BF0,
                      simtime = 100, gs_days = gs_time, disc_parms = params_d, con_parms = params_d)
