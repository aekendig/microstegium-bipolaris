...
# summarize by plant group effects
alpha_parms <- growth_mod_parms %>%
  filter(substr(Parameter, 1, 4) == "coef") %>%
  mutate(Competitor = substr(Parameter, 7, 7),
         Treatment = if_else(str_detect(Parameter, "fung") == T, "fungicide", "control"),
         Estimate = if_else(Estimate > 0, 0, -10 * Estimate)) %>% # remove facilitation, make alphas positive, multiply by 10
  group_by(Competitor, Treatment) %>%
  summarise(Estimate = mean(Estimate)) %>%
  ungroup() %>%
  expand_grid(tibble(Focal = c("A", "F", "P"))) %>%
  transmute(Parameter = paste0("alpha_", Focal, Competitor),
            Parameter = if_else(Treatment == "fungicide", paste0(Parameter, "_fung"), Parameter),
            Description = c(rep("competitive effects of annual", 3),
                            rep("competitive effects of annual (disease free)", 3),
                            rep("competitive effects of first-year perennial", 3),
                            rep("competitive effects of first-year perennial (disease free)", 3),
                            rep("competitive effects of adult perennial", 3),
                            rep("competitive effects of adult perennial (disease free)", 3)),
            Estimate = Estimate,
            Lower = NA,
            Upper = NA,
            Units = "g^-1^",
...
inv_dat <- modF %>%
  ...
  mutate(stage = "Ev established",
         time = time - (max(time) - 6)) %>%
  full_join(modA_inv %>%
              filter(time <= 14) %>%
              mutate(stage = "Mv invasion",
                     time = time + 11)) %>%
  filter(species %in% c("Annual biomass", "Perennial adult biomass", "Perennial first-year biomass")) %>%
...
# figure
ggplot(inv_dat, aes(x = time, y = N)) +
  geom_line(aes(color = Species))

#### start here: format figure ####
