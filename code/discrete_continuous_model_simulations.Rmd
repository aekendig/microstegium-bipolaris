...
lit_parms <- tibble(Parameter = c("beta_AC", "beta_FC", "beta_PC", 
                    ...
                    Description = c("litter transmission to annuals",
                                    ...
                                    "biomass mortality"),
                     Estimate = c(rep(0.001, 3), rep(10, 3), rep(1e-20, 3), 5500, 0.9, 0.15, 0.05, 0.59, 0.005),
                     Source = c(rep("NA", 9), "Benitez et al. 2020", "NA", "Redwood et al. 2018", "Garrison and Stier 2010", "DeMeester and Richter 2010", "NA")) %>%
  mutate(Lower = NA_real_,
         Upper = NA_real_,
         Units = c(rep ("day^-1^ g^-1^", 3), rep ("g", 3), rep("day^-1^", 3), "g^-1^", "day^-1^", rep("NA", 2), "NA", "day^-1^"))
...
cont_mod_fun <- function(t, x, params) {
  ...
  dLogBAdt <- r_A * (1 - alpha_AA * B_A - alpha_AF * B_F - alpha_AP * B_P) - m - v_A * I_A / B_A
  dLogBFdt <- r_F * (1 - alpha_FA * B_A - alpha_FF * B_F - alpha_FP * B_P) - m - v_F * I_F / B_F
  dLogBPdt <- r_P * (1 - alpha_PA * B_A - alpha_PF * B_F - alpha_PP * B_P) - m - v_P * I_P / B_P
  dIAdt <- (beta_AC * S_A * C + beta_AA * S_A * I_A + beta_AF * S_A * I_F + beta_AP * S_A * I_P)/(k_A + B_A) - (m + v_A) * I_A
  dIFdt <- (beta_FC * S_F * C + beta_FA * S_F * I_A + beta_FF * S_F * I_F + beta_FP * S_F * I_P)/(k_F + B_F) - (m + v_F) * I_F
  dIPdt <- (beta_PC * S_P * C + beta_PA * S_P * I_A + beta_PF * S_P * I_F + beta_PP * S_P * I_P)/(k_P + B_P) - (m + v_P) * I_P
  dDdt <- v_A * I_A
  dCdt <- h * D - b * C
...
disc_mod_fun = function(AS0, AI0, F0, P0, L0, C0, IA0, simtime, gs_days, disc_parms, con_parms){
  ...
  for(t in 1:(simtime - 1)){	
    ...
    LogB_P0 <- ifelse(P[t] > 0, log(b_P * P[t]), log(1e-10))
  ...
  # seedling establishment
    ...
    LogB_P0 <- ifelse(P[simtime] > 0, log(b_P * P[simtime]), log(1e-10))
...
# input values
modA <- disc_mod_fun(AS0 = 10, AI0 = 0, F0 = 0, P0 = 0, L0 = 0, C0 = as.numeric(parms[parms$Parameter == "h", "Estimate"]), IA0 = 0, simtime = 50, gs_days = 160, disc_parms = parms, con_parms = parms)
...
# input values
modF <- disc_mod_fun(AS0 = 0, AI0 = 0, F0 = 10, P0 = 0, L0 = 0, C0 = as.numeric(parms[parms$Parameter == "h", "Estimate"]), IA0 = 0, simtime = 50, gs_days = 160, disc_parms = parms, con_parms = parms)
...
# input values
modP <- disc_mod_fun(AS0 = 0, AI0 = 0, F0 = 0, P0 = 10, L0 = 0, C0 = as.numeric(parms[parms$Parameter == "h", "Estimate"]), IA0 = 0, simtime = 50, gs_days = 160, disc_parms = parms, con_parms = parms)
...
# input values
modT <- disc_mod_fun(AS0 = 10, AI0 = 0, F0 = 10, P0 = 10, L0 = 0, C0 = as.numeric(parms[parms$Parameter == "h", "Estimate"]), IA0 = 0, simtime = 50, gs_days = 160, disc_parms = parms, con_parms = parms)
...
# perennial only
modL1 <- disc_mod_fun(AS0 = 0, AI0 = 0, F0 = 10, P0 = 0, L0 = 0, C0 = 0, IA0 = 0, simtime = 100, gs_days = 160,
...
# why is the drop in annual biomass so large? What are the C values?
```
