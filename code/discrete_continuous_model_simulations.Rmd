...
growth_mod_parms <- as_draws_df(growthD2Mod2)  %>%
  ...
  transmute(Parameter = parameter,
            Description = c("annual-annual competition",
                            "annual-annual competition (disease-free)",
                            "annual-first-year perennial competition",
                            "annual-first-year perennial competition (disease-free)",
                            "annual-adult perennial competition",
                            "annual-adult perennial competition (disease-free)",
                            "first-year perennial-annual competition",
                            "first-year perennial-annual competition (disease-free)",
                            "first-year perennial-first-year perennial competition",
                            "first-year perennial-first-year perennial competition (disease-free)",
                            "first-year perennial-adult perennial competition",
                            "first-year perennial-adult perennial competition (disease-free)",
                            "adult perennial-annual competition",
                            "adult perennial-annual competition (disease-free)",
                            "adult perennial-first-year perennial competition",
                            "adult perennial-first-year perennial competition (disease-free)",
                            "adult perennial-adult perennial competition",
                            "adult perennial-adult perennial competition (disease-free)",
                            "annual growth rate",
...
xstart <- c(LogB_A = log(0.767 * 0.949/(1 + 0.0012 * 1.39e3) * 0.031 * 3e4 +
                           0.306 * 0.949/(1 + 0.0012 * 1.39e3) * 0.031 * 1.64e4),
            LogB_F = log(0.24 * 0.553/(1 + 5e-4 * 1.39e3) * 0.004 * 1.05e-30),
            I_A = 0, I_F = 0, D = 0,
            C = 1.86e4 + as.numeric(parms[parms$Parameter == "h", "Estimate"]) * 5.49e2)
...
outTS %>%
  filter(plant != "C") %>%
  ggplot(aes(x = time, y = log(pop), color = plant, linetype = state)) +
  geom_line() +
  theme_classic() +
  theme(legend.key.width = unit(1.5, 'cm')) +
  labs(x='Time (days)', y='Biomass (g)')
...
modF_inv <- disc_AFP_mod(AS0 = filter(modA_fin, species == "Susceptible annual")$N, 
                      ...
                      BP0 = 0, 
                      BF0 = 0, 
                      simtime = 100, gs_days = gs_time, disc_parms = parms, con_parms = parms)
...
params_df <- params %>%
  filter(!Parameter %in% c("alpha_FF", "alpha_FP", "alpha_PF", "alpha_PP", "alpha_AA_dis", "alpha_AF_dis", "alpha_AP_dis", "alpha_FA_dis", "alpha_PA_dis")) %>%
  mutate(Estimate = case_when(Parameter %in% c("beta_AC", "beta_AA", "beta_AF", "beta_AP", "beta_FA", "beta_PA", "p1") ~ 0,
                              Parameter == "p0" ~ -100,
                              TRUE ~ Estimate),
         Parameter = str_replace(Parameter, "_dis", ""))
