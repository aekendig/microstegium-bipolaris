...
modL_fig_dat <- modL %>%
  ...
  full_join(modL %>%
              ...
              mutate(plant_type = "Perennial biomass")) %>%
  full_join(modL %>%
    ...
    full_join(modL %>%
              ...
              mutate(plant_type = "Perennial density")) %>%
  full_join(modL %>%
              filter(species %in% c("Annual", "Perennial first-year", "Perennial adult")) %>%
              mutate(N = log(N),
                     plant_type = "Log density")) %>%
...
# figure
ggplot(modL_fig_dat, aes(x = time, y = N)) +
  geom_line(aes(color = plant_group, linetype = scenario), size = 0.8) +
...
# Mv biomass
modL %>%
  filter(species == "Annual biomass" & time == 300 & scenario != "Ev alone") %>%
  pivot_wider(names_from = scenario,
              values_from = N) %>%
  mutate(disease_change = (invasion - disease) / invasion)

```

# Virulence simulation
#### start here ####
# vary Ev virulence too
# may not need transmission
