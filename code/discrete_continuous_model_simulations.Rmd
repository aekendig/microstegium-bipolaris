...
parms_export <- parms %>%
  ...
  filter(str_detect(Parameter, "alpha_P") == F & str_detect(Parameter, "alpha_F") == F &
           !(str_detect(Parameter, "beta") == T & Estimate == 0))
...
# parameter values
parm_mult <- c(0, 1, 5, 10)
parm_var <- list("p1", "v_A", c("v_A", "v_F", "v_P"))
...
for(i in 1:length(parm_var)){
  ...
  for(j in 1:length(parm_mult)){
    
    # update parameters
    parms_sim <- parms %>%
      mutate(Estimate = case_when(Parameter %in% parm_var[[i]] ~ Estimate * parm_mult[j],
                                  TRUE ~ Estimate))
...
modF_dat <- bind_rows(modF_sim) %>%
  mutate(parm_val_char = paste0("x", parm_val) %>%
           fct_relevel("x0", "x1", "x5"),
         parm_char = case_when(parm == "p1" ~ "Mv germination virulence",
...
line_pal = c("dotted", "solid", "dashed", "dotdash")
...
inv_bio_fig_B <- ggplot(inv_bio_dat_B, aes(x = time, y = N)) +
  ...
  scale_linetype_manual(values = line_pal, guide = "none") +
  labs(x = "Time (years)", y = expression(paste("Biomass (g/", m^2, ")", sep = ""))) +
  fig_theme +
...
plot_grid(inv_bio_fig_A, inv_bio_fig_B, inv_bio_fig_C,
          labels = LETTERS[1:3],
          nrow = 1)
dev.off()
