...
growth_mod_parms <- as_draws_df(growthD2Mod2)  %>%
  ...
  transmute(Parameter = parameter,
            ...
            Estimate = case_when(parameter %in% c("r_A", "r_F", "r_P") ~ estimate/160, # r may not be sig, but we need an estimate
                                 ...
                                 TRUE ~ estimate),
            Estimate = case_when(substr(parameter, 1, 5) == "alpha" ~ (Estimate + alpha_add) * alpha_mult, # alpha adjustments
                                 TRUE ~ Estimate),
...
# input values
times <- seq(0, gs_time)
xstart <- c(LogB_F = log(0.24 * 0.002 * 0.004 * 915287),
            LogB_P = log(0.88 * 46754), 
            I_F = 0, I_P = 0, D = 0,
            C = 0)
...
# input values
modP <- disc_perennial_mod(F0 = 0, P0 = 10, L0 = 0, C0 = as.numeric(parms[parms$Parameter == "h", "Estimate"]), IF0 = 0, IP0 = 0, BP0 = as.numeric(parms[parms$Parameter == "b_P", "Estimate"]) * 10, BF0 = 0, simtime = 100, gs_days = 160, disc_parms = parms, con_parms = parms)
