...
# file: elymus_adult_gs_survival_model_2019_density_exp
# author: Amy Kendig
# date last edited: 10/23/20
# goal: estimate Elymus adult growing season survival based on density and fungicide treatments
...
# initial fit
evAGsSurvD2Mod1 <- brm(survival ~ fungicide * (mv_seedling_density + ev_seedling_density + ev_adult_density) + (1|site),
                       data = evAGsSurvD2Dat,
                       family = bernoulli,
...
summary(evAGsSurvD2Mod1)

# increase chains
evAGsSurvD2Mod2 <- update(evAGsSurvD2Mod1, chains = 3)
summary(evAGsSurvD2Mod2)
plot(evAGsSurvD2Mod2)
pp_check(evAGsSurvD2Mod2, nsamples = 50)
...
# simulate fit
fitDat <- simDat %>%
  mutate(survival = fitted(evAGsSurvD2Mod2, newdata = ., re_formula = NA, type = "response")[, "Estimate"],
         survival_lower = fitted(evAGsSurvD2Mod2, newdata = ., re_formula = NA, type = "response")[, "Q2.5"],
         survival_upper = fitted(evAGsSurvD2Mod2, newdata = ., re_formula = NA, type = "response")[, "Q97.5"])
...
# fit figure
(evAGsSurvD2Plot <- ggplot(fitDat, aes(background_density, survival, color = treatment, fill = treatment)) +
...
save(evAGsSurvD2Mod2, file = "output/elymus_adult_gs_survival_model_2019_density_exp.rda")
save(evAGsSurvD2Plot, file = "output/elymus_adult_gs_survival_figure_2019_density_exp.rda")
