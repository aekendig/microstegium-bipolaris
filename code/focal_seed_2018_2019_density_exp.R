...
seedSevD1Coef2 <- seedSevD1Coef[[1]] %>%
  ...
  select(year, focal, response, variable, Estimate:CI.Upper) %>%
  arrange(desc(focal), response, variable)
...
seedD2Mod <- update(seedD1Mod, newdata = seedD2Dat3, control = list(max_treedepth = 15))
