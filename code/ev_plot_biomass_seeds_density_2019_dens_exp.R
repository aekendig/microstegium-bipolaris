...
save(evASeedDensMod, file = "output/evS_plot_seed_density_model_2019_density_exp.rda")
...
ggplot(evPredD2Dat, aes(x = density, y = value)) +
  ...
  stat_summary(data = figD2Dat, geom = "errorbar", fun.data = "mean_cl_boot", width = 0, 
               position = position_dodge(dodge_width), aes(color = treatment)) +
  stat_summary(data = figD2Dat, geom = "point", fun = "mean", size = 2, 
               position = position_dodge(dodge_width), aes(color = treatment)) +
