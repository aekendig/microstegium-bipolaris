...
# file: elymus_adult_seed_production_fung_model_2019_density_exp
# author: Amy Kendig
...
evASeedD2Dat0 <- evASeedD2Dat %>%
  filter(background == "none")

# initial fit
evASeedFuD2Mod1 <- brm(log_seeds ~ fungicide + (1|site),
                  data = evASeedD2Dat0, family = gaussian,
...
summary(evASeedFuD2Mod1)

# increase chains and adapt delta
evASeedFuD2Mod2 <- update(evASeedFuD2Mod1, chains = 3,
                     control = list(adapt_delta = 0.99999))
summary(evASeedFuD2Mod2)
plot(evASeedFuD2Mod2)
pp_check(evASeedFuD2Mod2, nsamples = 50)
...
# simulate fit
fitDat <- simDat %>%
  mutate(log_seeds =fitted(evASeedFuD2Mod2, newdata = ., re_formula = NA)[, "Estimate"],
         log_seeds_lower = fitted(evASeedFuD2Mod2, newdata = ., re_formula = NA)[, "Q2.5"],
         log_seeds_upper = fitted(evASeedFuD2Mod2, newdata = ., re_formula = NA)[, "Q97.5"],
         seeds = exp(log_seeds) - 1,
         seeds_lower = exp(log_seeds_lower) - 1,
         seeds_upper = exp(log_seeds_upper) - 1)

# summarise by site
vizDat2 <- evASeedD2Dat0  %>%
...
(evASeedFuD2Plot <- ggplot(fitDat, aes(treatment, log_seeds, color = treatment)) +
...
#### output ####
save(evASeedFuD2Mod2, file = "output/elymus_adult_seed_fung_model_2019_density_exp.rda")
save(evASeedFuD2Plot, file = "output/elymus_adult_seed_fung_figure_2019_density_exp.rda")
