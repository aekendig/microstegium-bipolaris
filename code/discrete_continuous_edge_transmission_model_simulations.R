...
parms_edge <- read_csv("output/edge_transmission_model_parameters_2019_density_exp.csv")
...
# replace annual transmission with edge values
parms_all2 <- parms_all %>%
  filter(!(str_starts(Parameter, "beta") == T & str_ends(Parameter, "A") == T)) %>%
  full_join(parms_edge)

# control parameters
parms_ctrl <- parms_all2 %>%
  filter(Treatment == "control" | is.na(Treatment))

# fungicide parameters
parms_fung <- parms_all2 %>%
...
# save
write_csv(mod_inv_dis3, "output/discrete_edge_transmission_time_series.csv")
mod_inv_dis3 <- read_csv("output/discrete_edge_transmission_time_series.csv") %>%
...
# figure of discrete results
ggsave("output/discrete_edge_transmission_simulation_figure.pdf", 
