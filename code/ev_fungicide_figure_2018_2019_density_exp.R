...
growthFig <- ggplot(growthDat, aes(treatment, pred, color = treatment)) +
  geom_errorbar(aes(ymin = .lower, ymax = .upper), width = 0) +
  geom_point(size = 2) +
  ylab(expression(paste("Biomass (g ", plant^-1, ")", sep = ""))) +
  annotate(geom = "text", x = 0.5, y = max(growthDat$.upper), label = "Fungicide: +26%", hjust = 0, vjust = 1, size = 2.5) +
  scale_color_viridis_d(end = 0.5) +
  fig_theme +
  theme(axis.text.x = element_blank())

compFig <- ggplot(compDat, aes(treatment, pred, color = treatment)) +
  geom_errorbar(aes(ymin = .lower, ymax = .upper), width = 0) +
  geom_point(size = 2) +
  ylab(expression(paste(italic("M. vimineum "), alpha, sep = ""))) +
  annotate(geom = "text", x = 1.4, y = max(compDat$.upper), label = "Fungicide: -90%", hjust = 0, size = 2.5, vjust = 1) +
  scale_color_viridis_d(end = 0.5) +
  fig_theme +
  theme(axis.text.x = element_blank())

germFig <- ggplot(germDat, aes(treatment, pred, color = treatment)) +
  geom_errorbar(aes(ymin = .lower, ymax = .upper), width = 0) +
  geom_point(size = 2) +
  ylab("Germination") +
  annotate(geom = "text", x = 1.4, y = max(germDat$.upper), label = "Fungicide: -30%", hjust = 0, size = 2.5, vjust = 1) +
  scale_color_viridis_d(end = 0.5) +
  fig_theme
