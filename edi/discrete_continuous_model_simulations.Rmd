...
trans_mod_parms <- as_draws_df(sevD2Mod)  %>%
  ...
  mean_hdi(estimate) %>%
  ungroup() %>%
  mutate(estimate = case_when(.lower < 0 & .upper < 0 ~ estimate,
                                 .lower > 0 & .upper > 0 ~ estimate,
                                 TRUE ~ 0), # convert non-sig estimates to zero
         Parameter = substr(parameter, 1, 7)) %>%
  group_by(Parameter) %>%
  mutate(maxEst = max(estimate),
         maxEst = if_else(maxEst == 0 & str_detect(parameter, "fung") == T, 1, maxEst)) %>%
  ungroup() %>%
...
mv_germ_parms2 <- mv_germ_parms %>%
  add_row(Parameter = "g_I_S",
          Description = "annual germination loss due to infection",
          Estimate = 1 - (filter(mv_germ_parms, Parameter == "g_I")$Estimate / filter(mv_germ_parms, Parameter == "g_S")$Estimate),
          Lower = NA, Upper = NA, Units = "NA", Source = "experiment")
...
lit_parms <- tibble(Parameter = c("h", 
                    ...
                    Description = c("inoculum addition to litter",
                                    ...
                                    "litter transmission to annuals",
                     ...
                     Estimate = c(5500, 0.59, 0.05*surv_adjust, 0.15*surv_adjust, rep(0.001, 3), rep(1e-3, 3), 0.5),
...
#### export ####
parms_export <- parms %>%
  mutate(Parameter = case_when(Parameter == "alpha_AA" ~ "alpha_iA",
                               Parameter == "alpha_AF" ~ "alpha_iF",
                               Parameter == "alpha_AP" ~ "alpha_iP",
                               TRUE ~ Parameter)) %>%
  filter(str_detect(Parameter, "alpha_P") == F & str_detect(Parameter, "alpha_F") == F &
           !(str_detect(Parameter, "beta") == T & Estimate == 0) & Parameter != "g_I_S")

write_csv(parms_export, "../output/discrete_continuous_model_parameters_2018_2019_density_exp.csv")
...
### Annual invades perennial functions
```{r AF-inv-fun, echo = F}
af_inv_fun_both <- function(f_parms, a_parms){
  
  # perennial alone
  modF_fun <- disc_AFP_mod(AS0 = 0, AI0 = 0, F0 = 10, P0 = 0, L0 = 0, C0 = as.numeric(parms[parms$Parameter == "h", "Estimate"]), IA0 = 0, IF0 = 0, IP0 = 0, BP0 = 0, BF0 = 0, simtime = 100, gs_days = gs_time, disc_parms = f_parms, con_parms = f_parms)
  
  # extract final values
  modF_fun_fin <- modF_fun %>%
  filter(time == max(time))
  
  # annual invades
  modA_inv_fun <- disc_AFP_mod(AS0 = 10, 
                      AI0 = 0, 
                      F0 = filter(modF_fun_fin, species == "Perennial first-year")$N, 
                      P0 = filter(modF_fun_fin, species == "Perennial adult")$N, 
                      L0 = filter(modF_fun_fin, species == "Litter")$N, 
                      C0 = filter(modF_fun_fin, species == "Conidia")$N, 
                      IA0 = 0, 
                      IF0 = filter(modF_fun_fin, species == "Perennial first-year infected biomass")$N, 
                      IP0 = filter(modF_fun_fin, species == "Perennial adult infected biomass")$N, 
                      BP0 = filter(modF_fun_fin, species == "Perennial adult biomass")$N, 
                      BF0 = filter(modF_fun_fin, species == "Perennial first-year biomass")$N, 
                      simtime = 25, gs_days = gs_time, disc_parms = a_parms, con_parms = a_parms)
  
  return(list(modF_fun, modA_inv_fun))
}

af_inv_fun_a <- function(a_parms){
...
### Annual invades perennial simulations
```{r AF-inv-sim, echo = F}
# need to run F-only-discrete 

# parameter values
parm_mult <- list(c(0, 1, 1.7), c(0, 1, 5, 10), c(0, 1, 5, 10))
parm_var <- list("g_I_S", "v_A", c("v_A", "v_F", "v_P"))
...
for(i in 1:length(parm_var)){

  # cycle through changes in parameter values
  for(j in 1:length(parm_mult[[i]])){
    
    # update parameters
    parms_sim <- parms %>%
      mutate(Estimate = case_when(Parameter %in% parm_var[[i]] ~ Estimate * parm_mult[[i]][j],
                                  TRUE ~ Estimate))
    
    # simulate dynamics
    sim_out <- af_inv_fun_both(parms_sim, parms_sim)
    
    # name of list element
    name <- paste0(i, j)
    
    # save 
    modF_sim[[name]] <- sim_out[[1]] %>%
      mutate(parm = paste(parm_var[[i]], collapse = ", "),
             parm_val = parm_mult[[i]][j])
    
    modA_inv_sim[[name]] <- sim_out[[2]] %>%
      mutate(parm = paste(parm_var[[i]], collapse = ", "),
             parm_val = parm_mult[[i]][j])
...
# collapse lists
modF_dat <- bind_rows(modF_sim) %>%
  mutate(parm_val_char = paste0(parm_val, "x") %>%
           fct_relevel("0x", "1x", "1.7x", "5x"),
         parm_char = case_when(parm == "g_I_S" ~ "Mv germination virulence",
                               parm == "v_A" ~ "Mv biomass virulence",
                               parm == "v_A, v_F, v_P" ~ "Ev + Mv biomass virulence") %>%
           fct_relevel("Mv germination virulence", "Mv biomass virulence"))

modA_inv_dat <- bind_rows(modA_inv_sim) %>%
  mutate(parm_val_char = paste0(parm_val, "x") %>%
           fct_relevel("0x", "1x", "1.7x", "5x"),
         parm_char = case_when(parm == "g_I_S" ~ "Mv germination virulence",
                               parm == "v_A" ~ "Mv biomass virulence",
...
# save data
write_csv(modF_dat, "../output/annual_invasion_simulations_established_perennial.csv")
write_csv(modA_inv_dat, "../output/annual_invasion_simulations_invading_annual.csv")
...
inv_bio_dat <- modF_dat %>%
  ...
  mutate(Species = word(species, 1) %>%
           ...
           fct_relevel("Ev")) %>%
  group_by(time, stage, Species, parm_char, parm_val_char) %>%
  summarise(N = sum(N)) %>%
  ungroup()

# divide parameters
inv_bio_dat_A <- inv_bio_dat %>%
  filter(parm_char == "Mv germination virulence")

inv_bio_dat_B <- inv_bio_dat %>%
  filter(parm_char == "Mv biomass virulence")

inv_bio_dat_C <- inv_bio_dat %>%
  filter(parm_char == "Ev + Mv biomass virulence")

```


### Annual invades perennial figure
```{r AF-inv-fig, echo = T}
# figure settings
fig_theme <- theme_bw() +
...
line_pal = c("dotted", "solid", "twodash", "dashed", "dotdash")

# legend
leg_fig <- ggplot(filter(inv_bio_dat, Species == "Ev"), aes(x = time, y = N)) +
  geom_line(aes(color = parm_char, linetype = parm_val_char)) +
  scale_color_viridis_d(guide = "none") +
  scale_linetype_manual(values = line_pal, name = "Parameter\nchange") +
  fig_theme +
  theme(legend.position = c(0.4, 0.67))

# figure
inv_bio_fig_A <- ggplot(inv_bio_dat_A, aes(x = time, y = N)) +
  geom_line(aes(color = Species, linetype = parm_val_char)) +
  facet_wrap(~ parm_char) +
...
inv_bio_fig_B <- ggplot(inv_bio_dat_B, aes(x = time, y = N)) +
  geom_line(aes(color = Species, linetype = parm_val_char)) +
  facet_wrap(~ parm_char) +
  scale_color_manual(values = col_pal) +
  scale_linetype_manual(values = line_pal[-3], guide = "none") +
  labs(x = "Time (years)", y = expression(paste("Biomass (g/", m^2, ")", sep = ""))) +
  fig_theme +
  theme(legend.position = c(0.18, 0.85))

inv_bio_fig_C <- ggplot(inv_bio_dat_C, aes(x = time, y = N)) +
  geom_line(aes(color = Species, linetype = parm_val_char)) +
  facet_wrap(~ parm_char) +
  scale_color_manual(values = col_pal) +
  scale_linetype_manual(values = line_pal[-3]) +
  labs(x = "Time (years)", y = expression(paste("Biomass (g/", m^2, ")", sep = ""))) +
  fig_theme +
  theme(legend.position = "none")
...
inv_bio_fig_A2 <- align_plots(inv_bio_fig_A, leg, axis = "l")
inv_bio_fig_A3 <- ggdraw(inv_bio_fig_A2[[1]]) + draw_plot(inv_bio_fig_A2[[2]])

tiff("../output/annual_invasion_simmulations.tiff", width = 18, height = 6, units = "cm", res = 300, compression = "lzw")
plot_grid(inv_bio_fig_A3, inv_bio_fig_B, inv_bio_fig_C,
          labels = LETTERS[1:3],
          nrow = 1)
dev.off()
